...
const View = ({text}: ViewPropsType) => {
    ...
    const textSlices = text.split(/\n/).map(line => {
        ...
        // Проблема в том, чтобы в ретурн попала строка со стилями не только для всей строки, но к каждому слову (например выделение одного конкретного слова)
        const getTextStyle = line.split(" ").map(word => {
